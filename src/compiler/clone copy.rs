...
impl State {
	...
	fn lookup(&mut self, keychain: &mut Vec<String>) -> Result<Point, String> {
		match self.pmap.get(&keychain.join(".")) {
			Some(v) => Ok(v.clone()),
			None => {
				if keychain.len() > 1 {
					keychain.swap_remove(keychain.len() - 2);
					self.lookup(keychain)
				} else {
